...
class FollowUserWorker {
    ...
    constructor() { }
    // 2. 
    static auto_follow_users() {
        // 1. Lookup all users
        db["User"].findAll()
            .then((users) => {
            // 2. For Each User Create a Queue
            users.forEach((user) => {
                user.auto_follow_queue = Queue("follow_queue_" + user.screen_name, twitter_streams_1.TwitterStreams.redis_url);
                // Graceful Shutdown
    ...
    static follow_user_processor(queue) {
        // 1. Follow user
                    ...
                    if (!error) {
                        ...
                        db["TweetAction"].create({
                            ...
                            return db.sequelize.query(" DELETE FROM \"TweetActions\" \
                                ...
                                "AND 	 action='new'", { type: db.sequelize.QueryTypes.SELECT });
                        })
        ...
        return new Promise((resolve, reject) => {
            if (this.twitter_clients[screen_name])
                ...
                var oauth_credentials = {
                    consumer_key: twitter_streams_1.TwitterStreams.CONSUMER_KEY,
                    consumer_secret: twitter_streams_1.TwitterStreams.CONSUMER_SECRET,
                    access_token_key: "",
                    access_token_secret: "",
                };
...
FollowUserWorker.twitter_clients = {};
exports.FollowUserWorker = FollowUserWorker;
//# sourceMappingURL=follow_user.js.map