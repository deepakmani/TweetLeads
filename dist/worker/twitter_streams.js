...
const follow_user_1 = require("./follow_user");
class TwitterStreams {
    constructor() {
        // setup_twitter_search();
    }
    static follow_users() {
        follow_user_1.FollowUserWorker.auto_follow_users();
        this.find_users()
            .then((users) => {
            // For each user
    ...
    static auto_track_followers(user) {
        // 1. 
    }
    static find_users() {
    ...
    static save_tweets(tweets, search_query) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let tweets_to_save = tweets.map((tweet) => {
                ...
                // has_profile_link 		= 5
                if (tweet.user.url)
                    tweet_to_save.tweet_score += 5;
                // has_profile_description 	= 4
                if (tweet.user.description)
                    tweet_to_save.tweet_score += 4;
                // has_profile_age  		= 3
                if (tweet.user.age)
                    tweet_to_save.tweet_score += 3;
                // is_known_source
                if (tweet.source.match(/twitter/i))
                    tweet_to_save.tweet_score += 5;
    ...
    static auto_search_twitter(screen_name, search_query) {
    ...
    static search_twitter(screen_name, search_query) {
        return new Promise((resolve, reject) => {
            // get_twitter_client
    ...
    */
    ...
    static auto_follow_tweet_user(user) {
        // 1. Setup queue				
        user.auto_follow_queue = Queue("follow_queue_" + user.screen_name, this.redis_url);
        // Graceful Shutdown
    ...
    static follow_tweet_users(user) {
    ...
    static select_tweet_users_to_follow(screen_name) {
        return new Promise((resolve, reject) => {
...
// 1. 
TwitterStreams.CONSUMER_KEY = "W4BsbvQJdEZtg27W0sAfAg";
TwitterStreams.CONSUMER_SECRET = "3zbIFGQJv5QZyyWCGjUXiQzygnyV3P4YW3W3DFkpP0U";
TwitterStreams.redis_url = process.env.REDIS_URL || "redis://h:p806e6846f9ab7ec0ebd98fd1053575781deef05c037e2bb583bf3ff363290485@ec2-54-81-19-119.compute-1.amazonaws.com:25849";
TwitterStreams.MAX_FRIENDS = 5000;
...
/*
    ...
    @input: tweet,
    */
TwitterStreams.markTweetsAsContacted = (tweets, user) => {
    // Get List of tweets
    // For each tweet
    db.sequelize.query("SELECT status_id  \
	 						 FROM \"Tweets\" \
	 						 WHERE status_id IN" + mentions, { type: db.sequelize.QueryTypes.SELECT })
        .then((status_ids) => {
